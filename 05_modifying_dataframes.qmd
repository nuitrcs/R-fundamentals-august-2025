...
# to change it back to the original weight in kg values, run the code below:
healthdata$weight <- healthdata$weight / 2.2
```
...
We can also change variables with the `mutate()` function from the dplyr package.  `mutate()` returns a copy of the data frame with the modification. `mutate()` can be used to modify column values, or to create a new column and populate it with values. To make this change affect the data frame for future operations, we need to save the result back to the data frame:

```{r}
# just return a modified copy of healthdata
mutate(healthdata, weight = weight * 2.2)  # convert kgs to lbs

# create a new column to populate with values
mutate(healthdata, weight_lbs = weight * 2.2) 

# save the change
healthdata <-mutate(healthdata, weight_lbs = weight * 2.2)  
```


## TRY IT

Create a new variable called `height_inches` which divides height values in cm by 2.54 and stores the result.

Change the id variable back to an integer type.
