...
Often, Googling errors or warnings can be very helpful! It's likely that someone else has gotten that error and you'll find help in the Internet, often in Stack Overflow.
...
Select the rows from the `evp` data below where the vehicle make is one of FIAT, DATS, or GEO.
...
Using the `evp` dataset, convert the columns `subject_race` and `subject_sex` into factors.
...
Let's import the data we'll be using. Similarly to `evp`, the data is from the [Stanford Open Policing Project](https://openpolicing.stanford.edu/data/) and includes vehicle stops by the Evanston police in 2017. In contrast to `evp`, the data is not limited to January.
...
It contains six main functions, each a verb, of actions you frequently take with a data frame: select, filter, mutate, group_by, summarize, and arrange. Here we're going to cover the first five. You can take a look at [arrange](https://dplyr.tidyverse.org/reference/arrange.html) on your own.
...
Order does matter, as the commands are executed in order. So this would give us an error, because we're trying to filder on a column that we didn't select:
...
### EXERCISE (Bonus Challenge)

Is there a difference in output between base R expression and `dplyr::filter` function outputs? How many rows does each output data frame have? Are they different? Why might that be?

Hint: Investigate how NAs are treated in base R and by filter.
...
police %>%
  ...
  summarize(
    warnings = sum(outcome == "warning"),  # count how many warnings
